...
FixStyle(passivate,FixPassivate)

#else

#ifndef LMP_FIX_PASSIVATE_H
#define LMP_FIX_PASSIVATE_H
...
class FixPassivate : public Fix {
 public:
  class NeighList *list;

  FixPassivate(class LAMMPS *, int, char **);
  ~FixPassivate();
  void handleOxygen(int atomIndex);
  void handleSilicon(int atomIndex);
  void handle1CoordinatedSilicon(int atomIndex, double *oxygen0);
  void handle2CoordinatedSilicon(int atomIndex, double *oxygen0, double *oxygen1);
  // void handle3CoordinatedSilicon(int atomIndex, int *oxygenNeighbors);
  void handle3CoordinatedSilicon(int atomIndex, double *oxygen0, double *oxygen1, double *oxygen2);
  void addNewAtoms();
  void createOHPair(double *oxygenPosition, double *siliconPosition);
  void find_maxid();
  void init();
  void findVectorInPlaneDefinedByNormalVector(double *normal, double *result);
  ...
  tagint maxtag_all;
  MyAtom *newAtomsList;
  double **oxygenPositions;
  int numNewAtoms;
  int num1CoordinatePassivated;
  int num2CoordinatePassivated;
  int num3CoordinatePassivated;
  int numOxygenPassivated;
};
